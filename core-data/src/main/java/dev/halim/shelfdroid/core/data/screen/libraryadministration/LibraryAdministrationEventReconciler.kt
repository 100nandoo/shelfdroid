 ...
 */
internal class LibraryAdministrationEventReconciler(
  ...
  private val currentLibraries: () -> List<LibraryAdministrationLibrary>,
) {
  ...
  fun registerLocalMutation(event: LibraryAdministrationLibraryEvent) {
    if (event.type == LibraryAdministrationLibraryEventType.REFRESHED) return
    synchronized(lock) {
  ...
  fun accept(event: LibraryAdministrationLibraryEvent): Boolean {
    // Each reconnect is a recovery boundary and must force a fresh authoritative snapshot.
    if (event.type == LibraryAdministrationLibraryEventType.REFRESHED) return true
  ...
  suspend fun reconcile(
    event: LibraryAdministrationLibraryEvent,
    ...
    mutationCoordinator.withMutation {
      ...
      if (event.type == LibraryAdministrationLibraryEventType.REMOVED) {
