...
data class LibraryAdminDirectory(
  val path: String,
  val name: String,
  val level: Int,
)

data class LibraryAdminFilesystem(
  val isPosix: Boolean,
  val directories: List<LibraryAdminDirectory>,
)

/** Settings shared by the Audiobookshelf book-library create form. */
data class LibraryAdminBookSettings(
...
data class LibraryAdminPodcastSettings(
  val coverAspectRatio: Int = SQUARE_COVER_ASPECT_RATIO,
  val disableWatcher: Boolean = false,
 ...
 */
data class LibraryAdminFinishThreshold(
  ...
  fun selectMode(percentCompleteMode: Boolean): LibraryAdminFinishThreshold {
    val selectedValue =
  ...
  fun updateValue(value: Int): LibraryAdminFinishThreshold {
...
val LibraryAdminBookSettings.finishThreshold: LibraryAdminFinishThreshold
  get() =
    LibraryAdminFinishThreshold(
      percentComplete = markAsFinishedPercentComplete,
      timeRemaining = markAsFinishedTimeRemaining,
    )

fun LibraryAdminBookSettings.withFinishThreshold(
  threshold: LibraryAdminFinishThreshold
...
val LibraryAdminPodcastSettings.finishThreshold: LibraryAdminFinishThreshold
  get() =
    LibraryAdminFinishThreshold(
      percentComplete = markAsFinishedPercentComplete,
      timeRemaining = markAsFinishedTimeRemaining,
...
fun LibraryAdminPodcastSettings.withFinishThreshold(
  threshold: LibraryAdminFinishThreshold
...
data class LibraryAdminMetadataSource(
  val id: String,
  val name: String,
...
val DEFAULT_LIBRARY_METADATA_SOURCES: List<LibraryAdminMetadataSource> =
  listOf(
      LibraryAdminMetadataSource("folderStructure", "Folder structure"),
      LibraryAdminMetadataSource(
        "audioMetatags",
        "Audio file meta tags OR ebook metadata",
      ),
      LibraryAdminMetadataSource("nfoFile", "NFO file"),
      LibraryAdminMetadataSource("txtFiles", "desc.txt & reader.txt files"),
      LibraryAdminMetadataSource("opfFile", "OPF file"),
      LibraryAdminMetadataSource("absMetadata", "Audiobookshelf metadata file"),
    )
    .asReversed()
...
data class LibraryAdminDraft(
  ...
  val bookSettings: LibraryAdminBookSettings = LibraryAdminBookSettings(),
  ...
  fun moveMetadataSource(id: String, delta: Int): LibraryAdminDraft {
...
sealed interface LibraryAdminCreateResult {
  data class Created(val library: LibraryAdminLibrary) : LibraryAdminCreateResult
