...
enum class LibraryAdminScheduleInterval(val presetCron: String? = null) {
  Custom,
  Daily,
  Every12Hours("0 */12 * * *"),
  Every6Hours("0 */6 * * *"),
...
enum class LibraryAdminScheduleMode {
  Simple,
  Advanced,
}
...
data class LibraryAdminSimpleSchedule(
  val interval: LibraryAdminScheduleInterval = LibraryAdminScheduleInterval.Custom,
  val hour: String = "0",
  val minute: String = "0",
  ...
  fun summary(): String? {
    ...
    return when (interval) {
      ...
      LibraryAdminScheduleInterval.EveryHour -> "Run every hour"
      LibraryAdminScheduleInterval.Every30Minutes -> "Run every 30 minutes"
      LibraryAdminScheduleInterval.Every15Minutes -> "Run every 15 minutes"
    }
  }
}

/** Draft state for the reusable create/edit Schedule section. */
data class LibraryAdminScheduleDraft(
  val enabled: Boolean = false,
  val mode: LibraryAdminScheduleMode = LibraryAdminScheduleMode.Simple,
  val simple: LibraryAdminSimpleSchedule = LibraryAdminSimpleSchedule(),
  ...
  val cronExpression: String?
    get() =
      if (!enabled) null
        ...
        when (mode) {
          LibraryAdminScheduleMode.Simple -> simple.toCronExpressionOrNull()
          LibraryAdminScheduleMode.Advanced -> advancedCronExpression.trim().ifBlank { null }
        }
      }
  ...
  val summary: String?
    get() =
      when {
        !enabled -> null
        mode == LibraryAdminScheduleMode.Simple -> simple.summary()
