...
object LibraryItemSerializer : KSerializer<LibraryItem> {
    ...
    override fun deserialize(decoder: Decoder): LibraryItem {
        ...
        val mediaType = jsonObject["mediaType"]?.jsonPrimitive?.contentOrNull
            ?: throw SerializationException("mediaType field is missing")

        val media = jsonObject["media"]?.let {
            when (mediaType) {
        ...
        return LibraryItem(
            ...
            mediaType = mediaType,
            media = media,
            libraryFiles = jsonObject["libraryFiles"]?.let { decoder.json.decodeFromJsonElement(it) }
                ?: emptyList()
        )
    }
