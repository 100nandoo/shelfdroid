...
This note documents how Audiobookshelf implements the admin notifications feature today so ShelfDroid can follow the real backend and web UI contract instead of guessing from the screen.

## Where The Feature Lives

- The web app exposes notifications as a dedicated admin config page at `/config/notifications`, not as part of the generic `/config` settings form.
...
- If a route includes `:id`, the middleware also resolves the notification object and returns `404` when it does not exist.
- Source:
...
`NotificationManager.getData()` returns `notificationData`, which is defined in `server/utils/notifications.js`.
...
- The socket is mainly relevant after backend-triggered send/test flows update timestamps and failure counts.
...
- This feature is separate from `/api/settings`; it is not part of the existing server settings PATCH contract.
- The minimum backend contract ShelfDroid needs is:
  ...
  - `DELETE /api/notifications/:id`
  - `GET /api/notifications/:id/test`
- `GET /api/notifications/test` is useful only if ShelfDroid wants parity with the special `onTest` helper buttons.
- A first ShelfDroid version can safely ignore `libraryId` selection because the upstream web UI does too.
